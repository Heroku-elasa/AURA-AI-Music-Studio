...
interface ProducerFinderProps {
  ...
  onSaveProvider: (provider: ProducerProfile) => void;
  onRemoveProvider: (provider: ProducerProfile) => void;
  onClearAllSaved: () => void;
  onNoteChange: (index: number, note: string) => void;
  handleApiError: (err: unknown) => string;
  isQuotaExhausted: boolean;
  allSpecialists: ProducerProfile[];
  onSpecialistsFound: (providers: ProducerProfile[]) => void;
  onClearAllDbProviders: () => void;
  triggerSearch: boolean;
  onSearchTriggered: () => void;
  onRequestResearch: (topic: string) => void;
  consultationResult: SongGenerationResult | null;
  symptoms: string; // This is the user's idea
}

type SortKey = 'relevanceScore' | 'city' | 'name';

const ProducerFinder: React.FC<ProducerFinderProps> = ({ 
    savedSpecialists,
    onSaveProvider,
    onRemoveProvider,
    onClearAllSaved,
    ...
    const handleSearch = useCallback(async () => {
        if (!consultationResult) {
            setError(t('producerFinder.validationError'));
            return;
        }
        setError(null);
    ...
    }, [consultationResult, idea, maxResults, t, handleApiError, onSpecialistsFound, language]);
    ...
    const isProviderSaved = useCallback((provider: ProducerProfile): boolean => {
    ...
    return (
        <section id="producer-finder" className="py-12 sm:py-16 space-y-12">
            <div className="text-center">
                <h2 className="text-3xl font-bold text-white">{t('producerFinder.title')}</h2>
                <p className="mt-2 text-gray-400 max-w-2xl mx-auto">{t('producerFinder.subtitle')}</p>
            </div>
            ...
            <div className="max-w-3xl mx-auto bg-gray-800/50 rounded-lg p-8 shadow-lg backdrop-blur-sm border border-white/10 space-y-6">
                <div>
                    <label htmlFor="max-results" className="block text-sm font-medium text-gray-300">{t('producerFinder.maxResults')} ({maxResults})</label>
                ...
                <div>
                    <button onClick={handleSearch} disabled={isLoading || !consultationResult || isQuotaExhausted}
                        ...
                        {isLoading ? t('producerFinder.finding') : isQuotaExhausted ? t('quotaErrorModal.title') : t('producerFinder.findButton')}
                    </button>
                    {!consultationResult && <p className="text-xs text-center mt-2 text-yellow-400">{t('producerFinder.validationError')}</p>}
                </div>
            </div>
            ...
            {savedSpecialists.length > 0 && (
                <div className="mt-12 space-y-8 animate-fade-in">
                    <div className="flex justify-between items-center">
                        <h3 className="text-2xl font-bold text-white">{t('producerFinder.savedTitle')}</h3>
                        <button onClick={onClearAllSaved} className="px-3 py-1 bg-red-800/70 hover:bg-red-700 text-white text-sm font-semibold rounded-md transition-colors">{t('producerFinder.clearAll')}</button>
                    </div>
                        ...
                        {savedSpecialists.map((provider, index) => (
                            <div key={`${provider.id}-${index}`} className="bg-gray-800/50 rounded-lg shadow-lg backdrop-blur-sm border border-white/10 p-6 flex flex-col">
                                ...
                                <div className="text-sm text-gray-300 space-y-2 mt-4 pt-4 border-t border-white/10">
                                    <p><strong>{t('producerFinder.bio')}:</strong> {provider.bio}</p>
                                    <p><strong>{t('producerFinder.relevance')}:</strong> <span className="font-bold text-teal-300">{provider.relevanceScore}%</span></p>
                                </div>
                                
                                <div className="mt-4 pt-4 border-t border-white/10 flex-grow">
                                    <label htmlFor={`notes-${index}`} className="block text-sm font-medium text-gray-300 mb-2">{t('producerFinder.notesLabel')}</label>
                                    <textarea id={`notes-${index}`} rows={3}
                                        ...
                                        placeholder={t('producerFinder.notesPlaceholder')} value={provider.notes || ''} onChange={(e) => onNoteChange(index, e.target.value)} />
                                </div>
                                <div className="mt-6">
                                     <button onClick={() => onRemoveProvider(provider)} className="w-full text-center bg-red-600 text-white font-semibold py-2 px-4 rounded-md hover:bg-red-700 transition-colors">{t('producerFinder.remove')}</button>
                                </div>
                            </div>
                        ))}
                    </div>
            ...
            <div className="mt-12 space-y-8">
                <div className="flex justify-between items-center flex-wrap gap-4">
                    <div>
                        <h3 className="text-2xl font-bold text-white">{t('producerFinder.crateTitle')}</h3>
                        <p className="text-sm text-gray-400">{t('producerFinder.crateSubtitle')}</p>
                    </div>
                    {allSpecialists.length > 0 &&
                        <button onClick={onClearAllDbProviders} className="px-3 py-1 bg-red-800/70 hover:bg-red-700 text-white text-sm font-semibold rounded-md transition-colors">{t('producerFinder.clearCrate')}</button>
                    }
                </div>
                        ...
                        {sortedProviders.length > 0 ? (
                            <>
                                <div className="flex justify-end">
                                    <label htmlFor="sort-key" className="text-sm text-gray-400 self-center mr-2">{t('producerFinder.rankBy')}:</label>
                                    <select id="sort-key" value={sortKey} onChange={(e) => setSortKey(e.target.value as SortKey)}
                                        ...
                                        <option value="relevanceScore">{t('producerFinder.sort.relevance')}</option>
                                        <option value="city">{t('producerFinder.sort.city')}</option>
                                        <option value="name">{t('producerFinder.sort.name')}</option>
                                    </select>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            rawTextResult ? (
                                <div className="p-6 bg-gray-800/50 border border-white/10 rounded-lg">
                                    <h4 className="font-semibold text-white mb-2">{t('producerFinder.parseErrorTitle')}</h4>
                                    <p className="text-sm text-gray-400 mb-4">{t('producerFinder.parseErrorSubtitle')}</p>
                                    <pre className="whitespace-pre-wrap bg-gray-900/50 p-4 rounded-md text-sm text-gray-300">{rawTextResult}</pre>
                                </div>
                            ) : (
                                !error && <div className="text-center text-gray-500 py-10 bg-gray-800/30 rounded-lg"><p>{t('producerFinder.crateEmpty')}</p></div>
                            )
                        )}
                    </div>
                )}
